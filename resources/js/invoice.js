...
var invoices_form = $('#invoices_form');
...
invoices_form.submit((event) => {
	event.preventDefault();

	if (send_data_to_server.value) {
		$.ajax({
			url: Laravel.invoices_store,
			type: 'POST',
			headers: {
			...
			setTimeout(() => {
				if (data.value) {
					...
					showAlert('success', message, () => {
						...
						let destiny_url = Laravel.invoice_pdf.replace('param', invoice_id);
						window.open(destiny_url);
						window.location.href = Laravel.invoices_index;
					});
				} else {
					loading_modal_btn.click();
...
window.verifyStock = function (html_element){
	let td_list = $().add(html_element).parents('tr').find('td');

	let item_id = Number(td_list.eq(1).find('select').val());

	let data_of_item = getItem(item_id);
	if (data_of_item.value) {
		if (data_of_item.data.stock < Number(html_element.value)) {
			let message = 'La cantidad digitada sobrepasa el stock';
			showAlert('warning', message, () => {
...
window.calculateOfItem = function(html_element){
	...
	if (data_of_item.value) {
		...
		let iva = !!data_of_item.data.item.is_iva ? subtotal * 0.12 : 0;
...
window.calculateAllItems = function(){
	...
	tr_list.each((tr_index, tr_element) => {
		...
		if (data_of_item.value) {
			...
			let detail_subdata = {
				name: data_of_item.data.item.name,
				quantity: quantity,
				price: price,
				subtotal: subtotal,
				iva_value: iva,
				total: total,
				item_id: data_of_item.data.item.id
			};
	...
	let invoice_code = $('#invoice_code').val().toString().trim();
	let payment_type_id = $('#payment_type_id').val().toString().trim();
	let customer_id = Number($('#customer_id').val());

	send_data_to_server['value'] = detail_centinela && !!invoice_code.length && !!detail_to_server.length;
...
window.getItem = function(item_id){
	if (!!item_id) {
		let data = [];
...
window.getOptionsForSelect = function (){
	...
	products.forEach((element, index) => {
		html += `
			<option value="${ element.id }">${ element.item.name }</option>
		`;
	});
	return html;
};